...
// ============================================================
// Variant D 改善版: データストーリーテリング + 円グラフ
// - 最新届け出セクション削除
// - 高校/大学比率のドーナツチャート追加
// - 都道府県別分布のドーナツチャート追加
// ============================================================
...
const ChangeIndicator = ({ value, suffix = '名' }: { value: number; suffix?: string }) => {
  const isPositive = value > 0;
  const color = isPositive ? '#2e7d32' : '#c62828';
  return (
    <Box component="span" sx={{ display: 'inline-flex', alignItems: 'center', gap: 0.25, color, fontWeight: 600, fontSize: '0.8rem' }}>
...
const NarrativeCard = ({
  headline, value, unit, change, changeLabel, sparkData, sparkColor, accentColor,
}: {
  headline: string; value: number | string; unit: string; change?: number; changeLabel?: string; sparkData?: number[]; sparkColor?: string; accentColor: string;
}) => {
  const theme = useTheme();
  const isDark = theme.palette.mode === 'dark';
  return (
    <Paper elevation={0} sx={{ p: 2.5, borderRadius: 3, border: `1px solid ${isDark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.05)'}`, position: 'relative' }}>
      <Typography variant="caption" sx={{ fontWeight: 600, color: accentColor, display: 'block', mb: 1 }}>{headline}</Typography>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
        <Box>
          <Box sx={{ display: 'flex', alignItems: 'baseline', gap: 0.25 }}>
            <Typography sx={{ fontSize: 32, fontWeight: 800, lineHeight: 1, fontVariantNumeric: 'tabular-nums', letterSpacing: '-0.02em' }}>{value}</Typography>
            <Typography sx={{ fontSize: 14, fontWeight: 500, color: theme.palette.text.secondary }}>{unit}</Typography>
          </Box>
          {change !== undefined && (
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, mt: 0.5 }}>
              <ChangeIndicator value={change} />
              {changeLabel && <Typography variant="caption" sx={{
                color: "text.disabled"
...
export default function VariantF() {
  ...
  const schoolTypeData = {
    labels: ['高校生', '大学生'],
    datasets: [{
      data: [mockStats.highschoolPlayers, mockStats.universityPlayers],
      backgroundColor: [highschoolColor, universityColor],
  ...
  const prefectureColors = [primaryColor, highschoolColor, universityColor, isDark ? '#90caf9' : '#1565c0', isDark ? '#ce93d8' : '#7b1fa2', isDark ? '#a5d6a7' : '#2e7d32', isDark ? '#ef9a9a' : '#c62828'];
  const topPrefectures = mockPrefectureRanking.slice(0, 5);
  const topTotal = topPrefectures.reduce((sum, p) => sum + p.count, 0);
  const othersCount = mockStats.totalPlayers - topTotal;

  const prefectureData = {
    labels: [...topPrefectures.map((p) => p.prefecture), 'その他'],
    datasets: [{
      data: [...topPrefectures.map((p) => p.count), othersCount],
      backgroundColor: [...prefectureColors.slice(0, 5), isDark ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.08)'],
      borderColor: isDark ? 'rgba(30,30,30,1)' : 'rgba(255,255,255,1)',
      borderWidth: 3,
      hoverOffset: 6,
    }],
  ...
  const doughnutOptions = {
    ...
    plugins: {
      legend: {
        ...
        labels: { color: theme.palette.text.secondary, font: { size: 12, weight: 600 as const }, padding: 16, usePointStyle: true, pointStyleWidth: 8 },
      },
      tooltip: {
        ...
        callbacks: {
          label: (ctx: { label?: string; parsed?: number; dataset?: { data?: number[] } }) => {
            const total = ctx.dataset?.data?.reduce((a: number, b: number) => a + b, 0) ?? 0;
    ...
    <Box sx={{ maxWidth: 1200, mx: 'auto' }}>
      ...
      <Box sx={{ mb: 3 }}>
        ...
        <Typography sx={{ fontSize: { xs: 22, md: 28 }, fontWeight: 700, lineHeight: 1.3 }}>
          今年の志望届提出は
          <Typography component="span" sx={{ color: primaryColor, fontWeight: 800, fontSize: 'inherit' }}>{mockStats.totalPlayers}名</Typography>
          。前年から
      ...
      <Box sx={{ display: 'flex', gap: 3, mb: 3, flexDirection: { xs: 'column', md: 'row' } }}>
        <Paper elevation={0} sx={{ p: 3, flex: 1, borderRadius: 3, border: `1px solid ${isDark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.05)'}` }}>
          <Typography variant="subtitle2" sx={{ fontWeight: 600, mb: 0.5 }}>高校生・大学生の内訳</Typography>
          <Typography
            variant="caption"
            sx={{
            ...
            高校生が全体の{Math.round((mockStats.highschoolPlayers / mockStats.totalPlayers) * 100)}%を占める。
          </Typography>
          <Box sx={{ height: 220, display: 'flex', justifyContent: 'center' }}>
            <Doughnut data={schoolTypeData} options={doughnutOptions} />
          </Box>
        </Paper>

        <Paper elevation={0} sx={{ p: 3, flex: 1, borderRadius: 3, border: `1px solid ${isDark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.05)'}` }}>
          <Typography variant="subtitle2" sx={{ fontWeight: 600, mb: 0.5 }}>都道府県別の分布</Typography>
          <Typography
            variant="caption"
            sx={{
              color: "text.secondary",
            ...
            {mockStats.topPrefecture}が{mockStats.topPrefectureCount}名で最多。{mockStats.totalPrefectures}都道府県から提出。
          </Typography>
          <Box sx={{ height: 220, display: 'flex', justifyContent: 'center' }}>
            <Doughnut data={prefectureData} options={doughnutOptions} />
          </Box>
        </Paper>
      </Box>
      ...
      {/* 都道府県ランキング */}
      <Paper elevation={0} sx={{ p: 3, mb: 5, borderRadius: 3, border: `1px solid ${isDark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.05)'}` }}>
        <Typography variant="subtitle2" sx={{ fontWeight: 600, mb: 0.5 }}>地域別ランキング</Typography>
        <Typography
          variant="caption"
          sx={{
            color: "text.secondary",
