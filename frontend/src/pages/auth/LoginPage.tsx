...
export const LoginPage: React.FC = () => {
  ...
  useEffect(() => {
    // ローカル環境チェック
    if (isLocalhost()) {
      log.info('🏠 ローカル環境: ログイン画面をスキップしてダッシュボードに遷移');
    ...
    if (isAuthenticated && user) {
      log.info('✅ 既にログイン済み: トップページに遷移', { username: user.username });
    ...
    try {
      ...
      if (err && typeof err === 'object' && 'nextStep' in err) {
        ...
        if (nextStep?.signInStep === 'CONFIRM_SIGN_IN_WITH_NEW_PASSWORD_REQUIRED') {
          log.info('Password change required (from error):', { username });
          setRequiresNewPassword(true);
          setLoading(false);
          return;
        }
  ...
  const handlePasswordChange = async (e: React.FormEvent) => {
    ...
    if (
      ...
      setError('パスワードは大文字・小文字・数字・特殊文字をすべて含めてください。');
      setLoading(false);
      return;
    }
        ...
        <CardContent sx={{ p: 4 }}>
          ...
          {requiresNewPassword ? (
            <Box component="form" onSubmit={handlePasswordChange}>
              ...
              >
                {loading ? <CircularProgress size={24} color="inherit" /> : 'パスワードを変更'}
              </Button>
            </Box>
          ) : (
            /* 通常のログインフォーム */
            <Box component="form" onSubmit={handleSubmit}>
              ...
              {/*
                「パスワードを忘れた場合」「新規登録」のリンクは削除した。
                - どちらも /auth/forgot-password・/auth/signup を指していたが、該当ルートも画面も
                  存在せず（App.tsx 未登録）、押すと 404 になるリンク切れだった
                - Cognito のホスト型UIは存在するが、本画面は Amplify の SRP 認証を直接呼ぶ実装で
                  OAuth のコールバック処理を持たないため、ホスト型UIへ飛ばすと復帰できない
                - 閲覧系ページは未ログインで公開済みのため、一般利用者にアカウントは不要。
                  アカウントは管理操作用に運営者が発行する
              */}
              <Box sx={{ textAlign: 'center' }}>
                <Typography variant="body2" color="textSecondary">
                  アカウントの発行・パスワードの再設定は運営者にお問い合わせください
                </Typography>
              </Box>
            </Box>
          )}
      ...
      {shouldShowDemoInfo() && (
        <Alert severity="info" sx={{ width: '100%', maxWidth: 400, m: 2 }}>
          <Typography variant="body2">
            <strong>開発用デフォルトアカウント:</strong>
            <br />
            ユーザー名: admin
            <br />
            パスワード: {import.meta.env.VITE_DEMO_PASSWORD || '設定されていません'}
            <br />
