...
export const TestPage: React.FC = () => {
  ...
  const testAPI = async () => {
    ...
    try {
      log.info('APIテスト開始');
      log.info('設定:', apiConfig);

      const healthData = await triggerHealth().unwrap();
      log.info('ヘルスチェック結果:', healthData);

      const highschoolPlayers = await triggerHighschool().unwrap();
      log.info('高校生データ:', highschoolPlayers.data.length, '件');

      const universityPlayers = await triggerUniversity().unwrap();
      log.info('大学生データ:', universityPlayers.data.length, '件');
  ...
  return (
    <Box sx={{
      ...
      <Paper sx={{ p: 2, mb: 2 }}>
        ...
        <Typography variant="body2">MODE: {import.meta.env.MODE}</Typography>
      </Paper>
