...
class FrontendLogger {
  ...
  private getLogConfig(): LogConfig {
    // 環境変数の取得（テスト環境対応）
    // Vitest環境ではimport.meta.envが正しく処理されるため直接使用
    // eslint-disable-next-line @typescript-eslint/ban-ts-comment
    // @ts-ignore
    const env = (typeof import.meta !== 'undefined' && import.meta.env) ? import.meta.env : { MODE: 'test', VITE_LOG_LEVEL: 'DEBUG' };
    ...
    if (env.VITE_LOG_LEVEL && enableConsole) {
      switch (env.VITE_LOG_LEVEL.toUpperCase()) {
        case 'DEBUG':
