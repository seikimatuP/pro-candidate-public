  ...
  test('should stack cards vertically on mobile', async ({ page }) => {
    ...
    if (cardCount >= 2) {
      ...
      if (firstCardBox && secondCardBox) {
        ...
        const isVerticalStack = secondCardBox.y > firstCardBox.y;
        const isFullWidth = viewportSize ? firstCardBox.width / viewportSize.width > 0.7 : false;
        ...
        console.log(`Card positions - First Y: ${firstCardBox.y}, Second Y: ${secondCardBox.y}`);
        console.log(
          `Card width ratio: ${viewportSize ? ((firstCardBox.width / viewportSize.width) * 100).toFixed(1) : 'unknown'}%`
        );
      }
    }
  });
