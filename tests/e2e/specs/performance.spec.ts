  ...
  test('should have acceptable Largest Contentful Paint (LCP)', async ({ page }) => {
    ...
    const metrics = await page.evaluate(() => {
      return new Promise(resolve => {
        ...
        // タイムアウト設定をdev環境向けに延長
        setTimeout(() => resolve(null), 10000); // eslint-disable-line no-undef
      });
    });
  ...
  test('should handle large datasets efficiently', async ({ page }) => {
    ...
    try {
      ...
      try {
        ...
        tableExists = true; // eslint-disable-line no-unused-vars
      } catch {
        // ローディング状態確認
        try {
  ...
  test('should render charts without blocking UI', async ({ page }) => {
    ...
    try {
      ...
      const chartCount = await page
        ...
        .count();
      chartFound = chartCount > 0;
    }

    // チャートが無くてもUIが応答することを確認
    ...
    if (chartFound) {
      ...
      // チャートデータ未取得でもUI応答性は確認済み（prod環境のネットワーク遅延考慮）
