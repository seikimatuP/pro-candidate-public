...
test.describe('Dashboard Scraping Date Test', () => {
  test('should display correct scraping execution date', async ({ page }) => {
    ...
    await networkHelper.gotoWithRetry('http://localhost:5173/');
    ...
    // スクレイピング実行日時の要素を探す
    const scrapingDateElement = await page.locator('text=スクレイピング実行').locator('..').locator('text=/\\d{4}年\\d{1,2}月\\d{1,2}日/').first();
    ...
    if (await scrapingDateElement.count() > 0) {
      ...
      console.log('スクレイピング実行日時が表示されていません（管理者権限が必要な可能性があります）');

      // データ更新日時を確認（代替）
      const dataUpdateElement = await page.locator('text=データ更新').locator('..').locator('text=/\\d{4}年\\d{1,2}月\\d{1,2}日/').first();
    ...
    // スクリーンショットを撮影して確認
    await page.screenshot({ path: 'tests/e2e/screenshots/dashboard-scraping-date.png', fullPage: true });
  });
});