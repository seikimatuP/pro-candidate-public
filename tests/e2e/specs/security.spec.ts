...
test.describe('Security Tests', () => {
  ...
  test('should not expose sensitive data in console', async ({ page }) => {
    ...
    // CSPエラーやシステムログは除外（セキュリティ問題ではない）
    const sensitivePatterns = [
      { pattern: 'password', exclude: ['content security policy'] },
      { pattern: 'token', exclude: ['content security policy', 'violates the following'] },
      { pattern: 'secret', exclude: ['content security policy'] },
    ];

    consoleLogs.forEach(log => {
      const logLower = log.toLowerCase();

      sensitivePatterns.forEach(({ pattern, exclude }) => {
        if (logLower.includes(pattern)) {
          // 除外パターンに該当する場合はスキップ
          const isExcluded = exclude.some(ex => logLower.includes(ex));
          if (!isExcluded) {
            expect(logLower).not.toContain(pattern);
          }
        }
      });
