...
// Cognito のE2E用アカウント。値は Doppler（e2e_dev / e2e_prod config）から
// COGNITO_USERNAME / COGNITO_PASSWORD として注入する。リポジトリに実値は置かない。
// 参照: docs/development/DOPPLER_SETUP.md, docs/aws/COGNITO_ADMIN_SETUP.md
function getCognitoCredentials(): { username: string; password: string } {
  // ユーザー名は秘密情報ではないので既定値を持たせる。パスワードは必ず外部注入。
  const username = process.env.COGNITO_USERNAME || process.env.E2E_USERNAME || 'admin';
  const password = process.env.COGNITO_PASSWORD || process.env.E2E_PASSWORD || '';

  if (!username || !password) {
    throw new Error(
      'E2E用Cognito認証情報が未設定です。Doppler の e2e_dev / e2e_prod config に ' +
        'COGNITO_USERNAME と COGNITO_PASSWORD を設定し、doppler run 経由で実行してください' +
        '（pnpm run test:e2e:dev / test:e2e:prod）。'
    );
  }
...
function getEnvironment(): 'dev' | 'prod' | 'local' {
  ...
  return 'local'; // デフォルトをlocalに変更（安全側に倒す）
}
...
setup('authenticate', async ({ page }) => {
  ...
  const config = getCognitoCredentials();
  const baseUrl = process.env.E2E_BASE_URL || 'http://localhost:5173';
