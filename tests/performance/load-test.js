...
// 負荷テスト対象の API ベースURL。
// E2E_API_URL（scripts/run-e2e-test.sh が注入）を優先し、旧名 E2E_API_BASE_URL も
// 後方互換で受け付ける。どちらも無ければ dev 環境にフォールバックする。
const DEFAULT_API_BASE_URL = 'https://2esje5au24.execute-api.ap-northeast-1.amazonaws.com/dev';
const API_BASE_URL = (
...
async function runLoadTest() {
  ...
  console.log(
    `- エラー率: ${((playersTest.errors / playersTest.requests.total) * 100).toFixed(2)}%`
  );
