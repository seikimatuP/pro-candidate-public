...
export class MonitoringConstruct extends Construct {
  ...
  constructor(scope: Construct, id: string, props: MonitoringConstructProps) {
    ...
    // X-Ray Service Map Metrics（未使用のため削除）

    // Cost monitoring metrics (estimated)
    ...
    // 2. コスト監視アラーム（現在のstageのみ）
    new cloudwatch.Alarm(this, 'CostAlarm', {
      metric: estimatedCost,
      threshold: 0.5, // $0.50以上で警告
      evaluationPeriods: 1,
      alarmDescription: `AWS ${props.stage} environment costs approaching $0.50 limit`,
      alarmName: `aws-cost-limit-alert-${props.stage}-cdk`,
    }).addAlarmAction(new cloudwatch_actions.SnsAction(this.alertTopic));

    // 3. Lambda無料枠使用量アラーム（現在のstageのみ）
    ...
    // 以下のアラームは無料枠を超えないようにコメントアウト
    // 必要に応じて有効化可能

    // // X-Ray エラー率アラーム
    // new cloudwatch.Alarm(this, 'XRayErrorAlarm', {
    //   metric: xrayErrorRate,
    //   threshold: 5, // 5%以上でアラート
    //   evaluationPeriods: 2,
    //   alarmDescription: 'X-Ray traced functions error rate is too high',
    //   alarmName: `xray-errors-${props.stage}`,
    // }).addAlarmAction(new cloudwatch_actions.SnsAction(this.alertTopic));

    // // 無料枠使用量アラーム（Lambda）
    // new cloudwatch.Alarm(this, 'FreeTierUsageAlarm', {
    //   metric: new cloudwatch.MathExpression({
    //     expression: '(lambda_gb_seconds / 400000) * 100',
    //     usingMetrics: {
    //       lambda_gb_seconds: new cloudwatch.Metric({
    //         namespace: 'AWS/Lambda',
    //         metricName: 'Duration',
    //         statistic: 'Sum',
    ...
    //   threshold: 80, // 80%以上で警告
    //   evaluationPeriods: 1,
    //   alarmDescription: 'Lambda free tier usage is approaching 80%',
    //   alarmName: `free-tier-lambda-${props.stage}`,
    // }).addAlarmAction(new cloudwatch_actions.SnsAction(this.alertTopic));
