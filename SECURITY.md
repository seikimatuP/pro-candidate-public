...
- 🔒 **Player Names**: Player lists, player detail, name search, and per-school player lists require a Cognito login **and** an `admin` claim in `cognito:groups`. This is enforced in both dev and prod.
- ✅ **Public Endpoints**: Only `GET /health`, `GET /statistics` (aggregates without names or school names), `GET /schools`, and `GET /years/available` are unauthenticated.
- 🔒 **Defense in Depth**: The API Gateway Cognito authorizer is backed by an independent `admin` claim check inside the Lambda handler.
- 🔒 **No Self Sign-up**: Cognito self sign-up is disabled; accounts are created by the operator.
...
- **Doppler** for secrets management (injected at run time via `doppler run`, never committed)

### Best Practices We Follow

1. **No Hardcoding Secrets**
   - All sensitive data stored in Doppler and injected as environment variables
...
- Dependencies: See `pnpm-lock.yaml` for exact versions
...
- 🔒 **選手の氏名**: 高校生一覧・大学生一覧・選手詳細・氏名検索・学校別選手は、Cognito ログインに加えて `cognito:groups` の `admin` が必須。dev / prod の両方で有効
- ✅ **認証不要のエンドポイント**: `GET /health`、`GET /statistics`（氏名・学校名を含まない集計のみ）、`GET /schools`、`GET /years/available` の 4 本のみ
- 🔒 **二重防御**: API Gateway の Cognito オーソライザーとは別に、Lambda 側でも `admin` クレームを検証
- 🔒 **セルフサインアップ無効**: アカウントは運営者が招待・作成する
...
- **Doppler** - 秘密情報の管理（`doppler run` で実行時に注入・バージョン管理外）

### 遵守しているベストプラクティス

1. **シークレットのハードコーディング禁止**
   - すべての機密データは Doppler で管理し、実行時に環境変数として注入
   - AWS 認証情報は SSO で管理
...
- Node.js: 22.x（Lambda ランタイム）
- 依存関係: `pnpm-lock.yaml` で正確なバージョン管理
