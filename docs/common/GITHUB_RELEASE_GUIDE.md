...
```yaml
# deploy-infra.yml ワークフロー（インフラ）
on:
  release:
    types: [published]

# deploy-frontend.yml ワークフロー（フロントエンド）
on:
  release:
    types: [published]
```
...
2. **Deploy Infrastructure**（`deploy-infra.yml`）と **Deploy Frontend**（`deploy-frontend.yml`）が並行して自動開始
3. **prod環境デプロイ**が実行される

> **⚠️ 2026-08-23 時点の注意**: prod にはまだ PR #1481（実名一覧の認証必須化）が反映されていない。
> 次のリリースで prod の公開範囲が変わる（実名一覧が未ログインで見られなくなる）ため、
> リリースノートに明記し、admin アカウントの用意（`scripts/ensure-cognito-admin.sh prod`）を
> 済ませてから publish すること。
...
1. リポジトリ > **Actions** タブ
2. **Deploy Infrastructure** / **Deploy Frontend** ワークフロー選択
3. 実行ステータス・ログ確認
...
- `deploy-infra.yml` の `rollback-on-failure` jobが自動実行
- 緊急通知・ログ出力
- 手動介入が必要な場合の指示
...
- [ ] **Deploy Infrastructure / Deploy Frontend ワークフロー成功**
- [ ] **CloudFormationスタック更新完了**
- [ ] **API Gatewayエンドポイント応答確認**（`GET /health` が 200、`GET /players` が未認証で 401）
- [ ] **フロントエンドアクセス確認**（CloudFront の invalidation 完了後）
- [ ] **admin アカウントでログインして実名一覧が見えるか確認**
