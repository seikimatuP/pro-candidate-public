...
パッケージマネージャは **pnpm**（npm 禁止）。lockfile はルートの `pnpm-lock.yaml` 1本だけなので、
`.github/dependabot.yml` も **ルート（`/`）の 1 エントリ**で pnpm ワークスペース全体
（`frontend/`・`pro-candidate-aws/`・ルート）を見る。ディレクトリ個別のエントリは
lockfile 未更新の重複PRを量産するため置かない。

| ecosystem        | directory | 間隔   | グルーピング                              |
| ---------------- | --------- | ------ | ----------------------------------------- |
| `npm`            | `/`       | weekly | minor / patch を1本に集約（major は個別） |
| `github-actions` | `/`       | weekly | major / minor / patch を1本に集約         |
...
#### 1.1 lockfile 破損（`ERR_PNPM_BROKEN_LOCKFILE`）

Dependabot の PR が短期間に複数マージされると `pnpm-lock.yaml` にキー重複が混入し、
CI 全体が停止することがある。テストが「実行される前に」失敗している場合はここを疑う。
...
- Dependabot PR の自動評価・自動マージ（`.github/workflows/dependabot-automerge.yml`。
  詳細は [Dependabot 自動マージガイド](./DEPENDABOT_AUTOMERGE_GUIDE.md)）
- セキュリティ修正の即座マージ（`security` ラベル付きPR）
...
- [pnpm audit 公式ガイド](https://pnpm.io/cli/audit)
