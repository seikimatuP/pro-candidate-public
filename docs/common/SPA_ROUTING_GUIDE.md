...
/auth/login            ← ログイン画面
/dashboard             ← / へのリダイレクト
/highschool-players    ← 高校生一覧（要ログイン・admin）
/university-players    ← 大学生一覧（要ログイン・admin）
/players               ← 選手管理（要ログイン・admin）
/schools               ← 学校管理（要ログイン・admin）
/scraping-history      ← スクレイピング履歴（要ログイン・admin）
```

認証が必要なルートは `frontend/src/components/auth/ProtectedRoute.tsx` の
`requireAdmin` で保護され、未ログインだと `/auth/login` へリダイレクトされる。
...
| **直接アクセス/リロード** | S3サーバー | S3に/auth/loginファイルを要求 | ❌ 404エラー |
...
```
1. ユーザーが /auth/login でリロード
2. S3に /auth/login ファイルをリクエスト
3. ファイルが存在しない → 404
4. S3がErrorDocument（index.html）を返す
5. index.htmlが読み込まれ、React Routerが起動
6. React Routerが /auth/login を認識してLoginPageを表示
```
...
配信の正面は CloudFront。動作確認も CloudFront のドメインで行う。

```bash
# 基本アクセス
curl https://dh2yk8y9mj9wl.cloudfront.net

# SPAルーティングテスト
curl https://dh2yk8y9mj9wl.cloudfront.net/auth/login
curl https://dh2yk8y9mj9wl.cloudfront.net/highschool-players
```
...
1. **ビルド**: `pnpm --filter frontend build`で本番最適化
2. **S3同期**: `aws s3 sync dist/ s3://BUCKET --delete`
3. **CloudFront invalidation**: 配信は CloudFront 経由のため、S3 に置いただけでは反映されない。`aws cloudfront create-invalidation` が必須（`deploy-frontend.yml` は invalidation の完了まで待つ）
4. **設定確認**: S3ウェブサイト設定の確認
