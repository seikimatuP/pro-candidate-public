...
このガイドは、Playwrightテスト失敗調査（v1.2.64）で判明した「Jest統合テストとPlaywright E2Eテストの混同問題」を防ぐための区別方法を説明します。

## 📊 テストスイート比較表

| 項目             | Jest統合テスト                                       | Playwright E2Eテスト                      |
| ---------------- | ---------------------------------------------------- | ----------------------------------------- |
| **実行コマンド** | `npm test`                                           | `npm run test:e2e:dev`                    |
| **対象**         | バックエンドコード・ユニット・統合                   | フロントエンドUI・認証・API統合           |
...
| **現在の状態**   | ❌ 123件失敗（GAS移行関連）                          | ✅ 正常動作（33/46テスト成功継続中）      |
| **失敗原因**     | `draftScraping.player_list_update is not a function` | 実際の失敗なし（混同による誤認）          |
...
- Jest統合テスト結果: `test-results.json` を確認
