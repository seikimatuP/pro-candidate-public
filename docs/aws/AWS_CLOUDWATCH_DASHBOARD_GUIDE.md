...
**URL**: https://ap-northeast-1.console.aws.amazon.com/cloudwatch/home?region=ap-northeast-1#dashboards:name=ProCandidate-Overview-dev

### 本番環境

**URL**: https://ap-northeast-1.console.aws.amazon.com/cloudwatch/home?region=ap-northeast-1#dashboards:name=ProCandidate-Overview-prod
...
4. `ProCandidate-Overview-{環境}` を選択
...
- **正常な状態**: prod のスクレイピング関数は、稼働期間内であれば定期実行により一定間隔でスパイク
- **注意が必要**:
  - API関数で異常に高い呼び出し（DDoS攻撃の可能性）
  - 稼働期間内なのにスクレイピング関数の呼び出しが停止（スケジューラーの問題）

> **前提**: 定期スクレイピングの EventBridge ルールは **prod にのみ**作成される。
> さらに SSM パラメータ `/pro-candidate/{stage}/scraping-schedule-period` の稼働期間内でなければ
> Lambda 側でスキップする（既定は空＝必ずスキップ）。
> したがって dev のダッシュボードや、稼働期間外の prod でスクレイピング呼び出しが 0 なのは正常。
...
- API関数エラー: 入力データの検証、認証の確認
  （401 は Authorization ヘッダー無し・トークン期限切れ、403 は `admin` グループ未所属。
...
2. IAM認証情報でログイン
3. CloudWatch → Dashboards → `ProCandidate-Overview-{env}`を選択
...
- **開発環境**: `arn:aws:sns:ap-northeast-1:123456789012:pro-baseball-alerts-dev`
- **本番環境**: `arn:aws:sns:ap-northeast-1:123456789012:pro-baseball-alerts-prod`
...
- [AWS リソース一覧](AWS_RESOURCE_LIST.md)
- [AWS アーキテクチャ構成図](AWS_ARCHITECTURE_DIAGRAM.md)
- [プロダクション監視設定](PRODUCTION_MONITORING_SETUP.md)
