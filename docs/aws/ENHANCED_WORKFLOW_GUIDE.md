...
Enhanced版（deploy-infra.yml）は、Simple Auth方式を維持しながらエンタープライズ級の高度機能を提供するCI/CDワークフローです。
...
| **トリガー**         | develop全体 | **パス条件**（`pro-candidate-aws/**`）                          |
...
1. ソースコードチェックアウト
2. pnpm 10.x / Node.js 22 セットアップ
3. 依存関係インストール
4. 🔍 TypeScript事前コンパイルチェック
5. 🧪 単体テスト（Vitest：バックエンド・フロントエンド）
6. AWS Simple Auth認証
7. 🔨 CDK TypeScriptビルド
8. CDK Bootstrap確認
9. 📋 CDK Diff生成
...
CDK のデプロイロールは `cloudformation:DescribeEvents` を持たないため、Early Validation の
フックで落ちると CDK のログだけでは原因が分からない。失敗時にワークフロー側の認証情報で
次を出力する。

- スタックの `StackStatus` / `StackStatusReason`
- 直近のスタックイベント（最大40件）
- チェンジセットの `Status` / `StatusReason` とフック結果
- Cognito の既存グループ一覧（Early Validation の重複作成チェック用）
...
3. `pnpm run test:e2e:<env>` 実行
4. Playwright レポートのアップロード

### Job 4: notify-deploy-email

**役割**: デプロイ結果のメール通知（`if: always()`）
...
- [CI/CD パイプライン全体概要](CI_CD.md) - 基本設定・全ワークフロー
- [GitHub シークレット設定ガイド](GITHUB_SECRETS_SETUP.md) - シークレット・IAM権限エラー対処
- [Cognito 管理者セットアップ](COGNITO_ADMIN_SETUP.md) - admin グループの用意
- [AWS リソース一覧](AWS_RESOURCE_LIST.md) - AWS構成

---

_最終更新: 2026-08-23_  
_Enhanced GitHub Actions workflow, 5-jobs architecture, advanced deployment features_
