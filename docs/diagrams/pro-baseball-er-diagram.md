...
- **型定義**: `shared/types/index.ts` の `PlayerData`（フロント・バックエンド共通）
- **都道府県の正規化**: `shared/prefectures.ts` に一本化。`GET /statistics` のバックエンド集計にも同じ正規化を適用する（対応表は `shared/data/university-prefecture.json`）
- **公開範囲**: 氏名・学校名を含むため、このエンティティを返す API（`GET /players` 系・`/schools/{school}/players`）は Cognito 認証＋`admin` グループが必須。未認証で公開されるのは氏名・学校名を含まない集計（`GET /statistics`）のみ
